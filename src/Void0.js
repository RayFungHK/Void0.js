(function(global) {
  ...
  var fn = {},
    ...
    container = doc.createElement('template'),
    ...
    regexValidTime = /^([0-9]{2}):([0-9]{2})(:([0-9]{2})(\.[0-9]{1,3})?)?$/,
  ...
  fn = {
    ...
    isIterable: function(object) {
      if (fn.isObject(object) && object !== null) {
    ...
    renewQS: function(data) {
      if (fn.isPlainObject(data)) {
        var url = win.location.protocol + "//" + win.location.host + win.location.pathname + '?' + fn.param(data);
    ...
    param: function(data) {
      ...
      if (data instanceof ElementCollection) {
        ...
        fn.each(data.entries(), function(name, value) {
          datagroup[name] = value;
        });
        data = datagroup;
      }
  ...
  fn.DateTime = function(value) {
    ...
    function parse(value) {
      ...
      if (!fn.isDefined(value) || !value) {
        return parse(Date.now());
      } else if (fn.isNumber(value)) {
        return parse(new Date(value));
      } else if (fn.isDefined(value.constructor) && value.constructor == Date) {
        dateObject = value;
        ...
        if (regexValidTime.test(dateString)) {
          timeDelimited = dateString.split(':');
          ...
          dateDelimited = dateString.replace(/\s+/, ' ').split(' ');
    ...
    self = {
      ...
      setTimezoneOffset: function(value) {
        var newTz = -getTimezoneOffset(value),
          diff = datetime.timezoneOffset - newTz;
        if (diff !== 0) {
    ...
    fn.each(['day', 'month', 'year', 'hour', 'minute', 'second', 'millis'], function(i) {
      var name = this.charAt(0).toUpperCase() + this.slice(1),
        prop = this;
      /**
    ...
    function CubicBezier(p1x, p1y, p2x, p2y) {
      ...
      if (fn.isString(p1x) && (p1x = p1x.trim())) {
        if (!pathCache[p1x]) {
          ...
          this.controls[self.controls.length - 1].p3 = {
            x: 1,
            y: 1
          };
  ...
  var ElementCollection = (function() {
    ...
    defaultPrototype = {
      push: ary.push,
      indexOf: function(elem, selector) {
        var context = (__.isDefined(selector)) ? this.find(selector) : this;
        return ary.indexOf.call(context, (__.isIterable(elem)) ? elem[0] : elem);
      },
      forEach: ary.forEach,
      some: ary.some,
      length: 0,
          ...
          if (fn.isDefined(value)) {
            ...
            if (this.length > 0) {
              ...
              if (owner.defaultView && owner.defaultView.getComputedStyle) {
                return owner.defaultView.getComputedStyle(elem, null).getPropertyValue(css);
      ...
      /**
       * Get the specified ElementCollection by index and return as an ElementCollection
       * @param	{int}	 index		Zero-based index at which to begin extraction.
       * @return {ElementCollection}
       */
      get: function(index) {
        if (!this.length) {
        ...
        if (this.length >= index) {
          return Void0(this[index]);
        }
        return Void0();
      },

      /**
       * Get the last element of the ElementCollection
       * @return {ElementCollection}
       */
      last: function() {
        if (!this.length) {
      ...
      is: function(selector) {
        ...
        elem = this[0];
        loaded = doc.body.contains(elem);
        if (!loaded) {
          context = elem.parentNode;
        }

        if (fn.isCallable(selector)) {
          return selector.call(elem);
        } else {
          if (!fn.isIterable(selector)) {
            selector = Void0(selector, context);
          }
      ...
      /**
       * Check the current matched set of elements against a selector,
       * element, or Void0 object and extract the element which is not matched
       * @param	{object} selector A string containing a selector expression to match elements against.
       * @return {boolean}
       */
        ...
        if (this.length) {
          fn.each(this, function() {
            fn.each(this.children, function() {
              if (!this._added) {
                if (!selector || __(this).is(selector)) {
                  this._added = true;
                  collection.push(this);
                }
              }
        ...
        if (fn.isDefined(value)) {
          fn.each(this, function() {
            ...
            if (regexCheckable.test(this.type)) {
              ...
              var checkbox = parent.find('input[type=' + this.type + '][name="' + this.name + '"]');

              if (checkbox.length === 1) {
                checkbox.checked(value);
              } else {
                value = !fn.isIterable(value) ? [value] : value;
                parent.find('input[type=' + this.type + '][name="' + this.name + '"]').checked(false).filter(function() {
                  return value != null && value.indexOf(this.value) !== -1;
                }).checked(true);
              }
            } else if (this.tagName.toLowerCase() == 'select') {
              if (this.type === 'select-multiple') {
                if (__.isCallable(value)) {
                  value = value.call(this, Void0(this).val());
                } else {
                  value = !fn.isIterable(value) ? [value] : value;
                }
              } else {
                if (__.isCallable(value)) {
                  ...
                  value = fn.isIterable(value) ? value.slice(0, 1) : value;
                }
              }

              Void0(this).find('option').prop('selected', false).removeAttr('selected').filter(function() {
              ...
              this.value = (fn.isCallable(value)) ? value.call(this, Void0(this).val()) : value;
              Void0(this).attr('value', this.value).change();
            }
          });
          return this;
        } else {
      ...
      clone: function(selector) {
        ...
        fn.each(this, function() {
          ...
          if (fn.isNative(this.cloneNode) && (!fn.isDefined(selector) || elem.is(selector))) {
            elementCollection.push(this.cloneNode(true));
          }
        });
        return elementCollection;
      },
      ...
      show: function(recovery) {
        ...
        fn.each(this, function(i, elem) {
          ...
          if (!recovery) {
            elem.css('display', function(i, value) {
              if (value === 'none') {
                return (this._defaultdisplay || 'block');
              }
              return this;
            });
          } else {
            elem.removeCss('display');
          }
        });
      ...
      getTemplate: function(returnAsElement) {
        if (this.length) {
          var elem = this[0];
          if (elem.nodeName.toLowerCase() == 'template') {
            return (returnAsElement) ? elem.content.cloneNode(true).children : Void0(elem.content.cloneNode(true).children);
          }
        }
        return null;
      },
      ...
      formelem: function() {
        ...
        if (this.length) {
          this.each(function() {
            var elem = this,
              formset = (elem.nodeName.toLowerCase() === 'form' && elem.elements) ? elem.elements : Void0(elem).find('input,textarea,select');

            Void0(formset).filter(function() {
              return (regexSubmitName.test(this.tagName) && !regexSubmitType.test(this.type) && !this.disabled);
            }).each(function() {
              collection.push(this);
            });
          });
      ...
      setvalue: function(data, replace) {
        var elem,
          formelem;
        if (this.length) {
          if (fn.isPlainObject(data)) {
            ...
            (function deep(data, parent) {
              fn.each(data, function(name, value) {
                var fieldname = (parent) ? parent + '[' + name + ']' : name;
                if (fn.isIterable(value) || fn.isPlainObject(value)) {
                  if (mapping[fieldname + '[]']) {
                    mapping[fieldname + '[]'].val(value);
                    delete mapping[fieldname + '[]'];
                  } else {
                    deep(value, fieldname);
                  }
                } else {
                  if (mapping[fieldname]) {
                    mapping[fieldname].val(value);
      ...
      formdata: function() {
        ...
        if (this.length) {
          elem = this[0];
          if (FormData && elem.tagName.toLowerCase() === 'form') {
          ...
          Void0(formset).filter(function() {
            ...
            value = (elem.nodeName.toLowerCase() === 'select' && elem.type === 'select-multiple') ? Void0(elem).val() : [elem.value];

            fn.each(value, function() {
              if (FormData) {
                formdata.append(elem.name, this);
              } else {
                formdata[elem.name] = this;
              }
            });
          });
      ...
      addRow: function(contents) {
        var elem,
          header = [],
          node,
          contents = (fn.isArray(contents)) ? contents : [];

        if (this.length) {
          elem = this[0];
          if (elem.tagName.toLowerCase() == 'table') {
            if (!(header = Void0(elem).find('thead > tr:first-child')).length) {
              header = Void0(elem).find('tr:first-child');
            }
          } else if (elem.tagName.toLowerCase() == 'tbody') {
            header = Void0(elem).parent('table').find('thead > tr:first-child');
          }

          if (header.length) {
            node = elem.appendChild(doc.createElement('tr'));
            fn.each(header.find('td, th'), function(index) {
              var col = doc.createElement('td');
              if (fn.isDefined(contents[index])) {
                (function run(col, object) {
                  if (fn.isString(object) || fn.isNumber(object)) {
                    col.innerHTML = object;
                  } else if (fn.isDOMElement(object)) {
                    __(col).append(object);
                  } else if (fn.isIterable(object)) {
                    fn.each(object, function() {
                      run(col, this);
                    })
                  }
                })(col, contents[index]);
    ...
    fn.each(['first', 'last'], function(index, name) {
      defaultPrototype['is' + name] = function() {
        if (this.length) {
          var elem = this.parent(),
            children = elem.children();
          if (elem.length) {
            return this.is(children[(index) ? children.length - 1 : 0]);
          }
        }
        return false;
      }
    ...
    fn.each({
      ...
      var direction = (name === 'scrollTop') ? 'y' : 'x',
              ...
              animationChain(this, function(resolve, reject) {
                ...
                function raf(timestamp) {
                  ...
                  fn.each(diff, function(i, object) {
                    Void0(object.elem).css(object.style, (object.org + cubicBezier.calculate(object.value, t)) + (object.hasUnit ? 'px' : ''));
                  });
          ...
          if (fn.isDefined(object)) {
            fn.each(this, function() {
              ...
              if (object === null) {
                ...
                if (fn.isString(object)) {
                  this.dataset[name] = object;
                } else {
    ...
    (function() {
      function createList(object) {
        ...
        if (fn.isDOMElement(object)) {
          ...
          return contents;
        } else if (fn.isIterable(object) && object.length) {
          return object;
        } else if (fn.isString(object)) {
          return Void0(object);
        }
      ...
      function insertElement(pair, prependElement, sibling) {
        ...
        if (source.length) {
          fn.each(target, function(i, el) {
            ...
            // If prepend(), prependTo() and before(), reverse the source list
            if ((!sibling && prependElement) || (sibling && !prependElement)) {
              source = ary.reverse.call(source);
            }

            (function loop(list) {
              fn.each(list, function(j, element) {
                if (fn.isString(element)) {
                  element = Void0(element);
                }
                ...
                if (__.isIterable(element) && element.length) {
                  loop(element);
                } else {
                  ...
                  if (targetNode) {
                    if ((targetNode.lastElementChild === el && !prependElement && sibling) || (!prependElement && !sibling)) {
    ...
    (function() {
      ...
      fn.extend(EventEmitter.prototype, {
        ...
        trigger: function(e) {
          ...
          // If the element is a selectbox, put the selected element into event object
          if (elem.tagName && elem.tagName.toLowerCase() === 'select') {
            e.selected = elem.options[elem.selectedIndex];
          }

          e.spg = e.stopPropagation;
          e.stopPropagation = function() {
            if (!stop) {
              if (__.isNative(e.spg)) {
                e.spg();
              }
              stop = true;
            }
            ...
            if (!namespaces.length) {
              if (object.selector) {
                ...
                if (fn.isCallable(object.selector)) {
                  ...
                  fireEvent = Void0(e.target).closest(object.selector).length > 0;
                }
              }

              if (fireEvent) {
                if (!object.callback.call(elem, e)) {
                  result = false;
                }
      ...
      function setupEvent(element, events, selector, callback) {
        ...
        if (!element.eventEmitters[eventName]) {
          ...
          if (/^(DOMContentLoaded|(on)?load)$/i.test(eventName) && (fn.isDocument(element) || fn.isWindow(element))) {
            ...
            var eventCallback = function(e) {
              var result = element.eventEmitters[eventName].trigger(e);
            ...
            if (/^(touch|scroll).*/.test(eventName)) {
              options =  {
                capture: true,
                passive: true
              };
        ...
        element.eventEmitters[eventName].register(namespaces, selector, callback);
          ...
          fn.each(this, function(i, elem) {
            fn.each(events, function() {
              var delimiter = this.split('.'),
                eventName = delimiter.shift(),
                namespaces = delimiter,
                eventObj = createEvent(elem, eventName);
  ...
  function buildHTML(html, source) {
    ...
    if (fn.isIterable(source)) {
      fn.each(container.content.children, function() {
